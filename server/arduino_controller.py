...
class ArduinoController:
    ...
    def send_score(self, score: int):
        ...
        if self.connected and self.serial:
            # Make sure the port is open; if not, try to reconnect
            if not getattr(self.serial, 'is_open', True):
                print("Le port série n'est pas ouvert, tentative de reconnexion...")
                self.connected = False
                try:
                    self.connect()
                except Exception as e:
            ...
            try:
                ...
                # Port locked / permission denied — give actionable advice and attempt a reconnect once
                print(f"Erreur d'envoi à l'Arduino (PermissionError): {pe}")
                print("→ Vérifiez qu'aucune autre application (Arduino IDE Serial Monitor, PlatformIO, Terminal) n'utilise le port série. Fermez-les puis réessayez.")
                try:
                    # Close and try to reconnect once
                    if self.serial:
                        try:
                            self.serial.close()
                        except Exception:
                    ...
                    self.connected = False
                    if self.connect():
                        print("Reconnexion réussie, ré-essai d'envoi...")
                        self.serial.write(message.encode())
                        return True
                except Exception as e:
                    print(f"Nouvel échec d'envoi après reconnexion: {e}")
                return False
